-- pg_cron + pg_net scheduled push notification jobs
-- Prerequisites (run manually in Supabase Dashboard):
--   Settings → Extensions → Enable: pg_cron, pg_net
--   Settings → Vault → Add secret named: service_role_key
--
-- All times are UTC. Israel is UTC+3 (summer) / UTC+2 (winter).
-- These times target ~UTC+3 (DST). Adjust ±1 for winter.
...
-- Missing form reminder - morning: 04:00 UTC = 07:00 IST (1hr after 06:00 shift)
SELECT cron.schedule(
  'push-form-reminder-morning',
  '0 4 * * *',
  $$
...
-- Missing form reminder - afternoon: 12:00 UTC = 15:00 IST (1hr after 14:00 shift)
SELECT cron.schedule(
  'push-form-reminder-afternoon',
  '0 12 * * *',
  $$
...
-- Missing form reminder - evening: 20:00 UTC = 23:00 IST (1hr after 22:00 shift)
SELECT cron.schedule(
  'push-form-reminder-evening',
  '0 20 * * *',
  $$
...
-- Cleaning parade notifications: 05:05 UTC = 08:05 IST
SELECT cron.schedule(
  'push-cleaning-notifications',
  '5 5 * * 1,3,6',
  $$
  SELECT net.http_post(
    url := current_setting('app.settings.supabase_url', true) || '/functions/v1/send-cleaning-notifications',
    headers := jsonb_build_object(
      'Content-Type', 'application/json',
      'Authorization', 'Bearer ' || current_setting('app.settings.service_role_key', true)
    ),
